...
\begin{tikzpicture}
% Center node
\node[process] (engine) {4.0 Compute Risk \&\\Recommendations};

% Left nodes positioned symmetrically
\node[entity, left=1.5cm of engine, yshift=1.25cm] (planned) {2.0 Launch Submission\\Request};
\node[datastore, left=1.5cm of engine, yshift=-1.25cm] (external) {D2: Event Index\\(Elasticsearch)};

% Right nodes positioned symmetrically
\node[datastore, right=1.5cm of engine, yshift=1.25cm] (risk) {Risk Score \&\\Alternative Dates};
\node[datastore, right=1.5cm of engine, yshift=-1.25cm] (conflicts) {5.0 Interactive\\Dashboard Display};
